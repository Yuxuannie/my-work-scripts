...
    logging.info("Starting path replacement process")
    for root, dirs, files in os.walk(base_dir):
        # Check if the current directory is part of a "Netlist" directory
        if "Netlist" in root.split(os.sep):
            continue  # Skip replacement in any Netlist directory or its subdirectories
 
...
 
    logging.info(f"Path replacement completed in {base_dir}")
 
...
 
def create_optional_directories(base_dir, rf_dir, dir_name):
    logging.info("Creating optional directories")
    optional_dirs = ['PMD', 'Postprocess']
    for opt_dir in optional_dirs:
        ref_path = os.path.join(rf_dir, dir_name, opt_dir)
        if os.path.exists(ref_path):
            dest_path = os.path.join(base_dir, opt_dir)
            os.makedirs(dest_path, exist_ok=True)
            logging.info(f"Optional directory {opt_dir} created at {dest_path}")
        else:
            logging.debug(f"Optional directory {opt_dir} not found at {ref_path}")
 
...
    required_dirs = ["Char", "Template", "Netlist", "Release", "LVF"]
    for req_dir in required_dirs:
        os.makedirs(os.path.join(base_dir, req_dir), exist_ok=True)
        logging.info(f"Required directory {req_dir} created.")
 
    # Create LVF subdirectories
    logging.info("Creating LVF subdirectories")
    lvf_subdirs = ["Char", "Template", "Netlist", "Release"]
    for subdir in lvf_subdirs:
        os.makedirs(os.path.join(base_dir, "LVF", subdir), exist_ok=True)
        logging.info(f"LVF subdirectory {subdir} created.")
 
    # Create optional directories
    create_optional_directories(base_dir, args.rf_dir, dir_name)
 
    logging.info("------------------------------------------------------")
    # Copy LPE netlist
    logging.info("Attempting to copy LPE netlist")
    lpe_netlist_path = find_lpe_netlist_path(args.rf_dir, dir_name, args.LPE_corner)
    if lpe_netlist_path:
        logging.info(f"LPE netlist found: {lpe_netlist_path}")
        copy_lpe_netlist(lpe_netlist_path, os.path.join(base_dir, "Netlist", args.LPE_corner))
        copy_lpe_netlist(lpe_netlist_path, os.path.join(base_dir, "LVF", "Netlist", args.LPE_corner))
    else:
        warning_logger.warning(f"LPE netlist not found for {args.LPE_corner} in {dir_name}, attempting fallback.")
        # Attempt to use fallback directory
        fallback_netlist_path = find_fallback_lpe_netlist_path(args.rf_dir, dir_name, args.new_c_code, args.LPE_corner)
        if fallback_netlist_path:
            logging.info(f"Using fallback LPE netlist: {fallback_netlist_path}")
            copy_lpe_netlist(fallback_netlist_path, os.path.join(base_dir, "Netlist", args.LPE_corner))
            copy_lpe_netlist(fallback_netlist_path, os.path.join(base_dir, "LVF", "Netlist", args.LPE_corner))
        else:
            warning_logger.warning(f"No LPE netlist available for {args.LPE_corner} in both {dir_name} and fallback.")
 
    logging.info("------------------------------------------------------")
    # Copy fix_ref.lib
    logging.info("Attempting to copy fix_ref.lib")
    find_and_copy_fix_ref(args.rf_dir, dir_name, os.path.join(base_dir, "Release"))
    find_and_copy_fix_ref(args.rf_dir, dir_name, os.path.join(base_dir, "LVF", "Release"))
 
...
        for section in ["Char", "Template", "Release"]:
            src_dir = os.path.join(args.rf_dir, dir_name, section)
            dst_dir = os.path.join(base_dir, section)
            if not os.path.exists(src_dir):
                warning_logger.warning(f"Source directory {src_dir} does not exist. Skipping.")
                continue
...
                        logging.info(f"Copied {item} to {dst_dir}")
 
...
 
    logging.info("------------------------------------------------------")
    # Replace paths, excluding any Netlist directory and its subdirectories
    logging.info("Replacing old paths with new paths (excluding Netlist directories)")
    replace_paths(base_dir, args.replacements)
 
    logging.info("------------------------------------------------------")
    # Rename netlist files to uppercase
    logging.info("Renaming netlist files to uppercase")
    netlist_dir = os.path.join(base_dir, "Netlist", args.LPE_corner)
    if os.path.exists(netlist_dir):
        for file_name in os.listdir(netlist_dir):
            base, ext = os.path.splitext(file_name)
            new_name = f"{base.upper()}{ext}"
            os.rename(os.path.join(netlist_dir, file_name), os.path.join(netlist_dir, new_name))
            logging.info(f"Renamed {file_name} to {new_name}")
 
    logging.info("------------------------------------------------------")
    # Copy Netlist to LVF
    logging.info("Copying Netlist to LVF directory")
    lvf_netlist_dir = os.path.join(base_dir, "LVF", "Netlist")
    if os.path.exists(netlist_dir):
        shutil.copytree(netlist_dir, lvf_netlist_dir, dirs_exist_ok=True)
        logging.info(f"Netlist copied to LVF directory {lvf_netlist_dir}")
 
    logging.info("------------------------------------------------------")
    # Generate simulation scripts for sf and fs corners
    if args.use_replace:
        logging.info("Generating simulation scripts for additional corners")
        for root, dirs, files in os.walk(base_dir):
            for file_name in files:
                if "ssgnp" in file_name:
                    for corner in args.corners:
                        if corner != "ssgnp":
                            new_file_name = file_name.replace("ssgnp", corner)
                            shutil.copy(os.path.join(root, file_name), os.path.join(root, new_file_name))
                            with open(os.path.join(root, new_file_name), 'r') as file:
                                content = file.read()
                            content = content.replace("ssgnp", corner).replace("SSGNP", corner.upper())
                            with open(os.path.join(root, new_file_name), 'w') as file:
                                file.write(content)
                            logging.info(f"Generated simulation script {new_file_name} for corner {corner}")
 
