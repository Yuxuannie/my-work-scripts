...
 
class MCData(object):
    def __init__(self, stat_file, arc_type, cell, pin, pin_dir, rel_pin, rel_pin_dir,
...
    # Required arguments
    required_arg_info = [
        ("--qa_directory=", ":", "The path to the QA directory, including the '/DECKS' "
                                "subdirectory."),
        ("--sensitivity_file=", ":", "The full path to the characterization sensitivity "
                                    "file"),
        ("--output_file=", ":", "The path to the output file where the spreadsheet is "
                               "written"),
    ]
    print("Arguments:")
...
    # Optional arguments
    optional_arg_info = [
        ("--bundle_mode", ":", "A flag that will allow the script to handle bundled "
                               "sensitivity files."),
...
 
    mc_obj = MCData(stat_file, arc_type, cell, pin, pin_dir, rel_pin, rel_pin_dir,
                    literal_when, logical_when, table_point, arc_id)
 
    return mc_obj
 
...
        matching_obj_list = findMatchingMCObjs(cell, pin, rel_pin, t_type, when,
                                               table_name, sigma, mc_objs_list,
                                               bundle_mode)
...
            variety_sigma = float(lib_data[cell][pin][rel_pin][t_type][when][t_sense][
                table_name][sigma][vec_point])*1e3
 
...
 
 
def findMatchingMCObjs(cell, pin, rel_pin, timing_type, when, table_name, sigma,
                       mc_objs_list, bundle_mode):
 
...
            print("ERROR: Couldn't compute the errors for path %s" % mc_obj.stat_file)
            if mc_obj.variety_sigma is None:
                print("This table point seems to be missing from the sensitivity file.")
...
 
        try:
            write_line = ','.join(["%s" for _ in range(len(header.split(',')))]) % (
                mc_obj.abs_error,
                float(mc_obj.rel_error)*100,
                "arc_{0}".format(mc_obj.arc_id),
                mc_obj.cell,
                mc_obj.arc_type,
                mc_obj.pin,
                mc_obj.pin_dir,
...
                "%s;%s" % (row, col),
                mc_obj.num_samples,
                mc_obj.variety_sigma,
                mc_obj.mc_sigma,
                mc_obj.mc_nominal,
                mc_obj.mc_percentile,
                mc_obj.mc_skewness,
                mc_obj.mc_kurtosis,
                mc_obj.effort)
...
                mc_obj.mc_nominal,
                mc_obj.mc_percentile,
                mc_obj.mc_percentile_lb,
                mc_obj.mc_percentile_ub,
                mc_obj.mc_skewness,
                mc_obj.mc_kurtosis,
                mc_obj.effort)
            write_buffer.append(write_line)
...
    lib_data = library_parser.parseLibraryFile(user_options['SENSITIVITY_FILE'],
                                               "variation")
 
...
    storeAllVarietyDataInMCObjs(mc_objs_list, lib_data, user_options['BUNDLE_MODE'])
 
    # Compute errors
...
    fileIO.writeBufferToFile(write_buffer, user_options['OUTPUT_FILE'], delimeter='\n')
 
 
