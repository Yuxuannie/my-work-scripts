...
 
 
def createQATemplateFile(template_info, valid_arc_types, output_file,
                         cell_pattern_list, template_deck_type,
                         arc_characteristics=None, max_num_when=1):
    """
    A function that first creates the arc_characteristics dictionary representing the
    different attributes/characteristics (needed for sensitization) of the arcs
    in the template, then writes this information to a summary file which can be
    modified and/or used down the flow.
 
    Args:
        template_info (charTemplateParser.classes.TemplateInfo): The dictionary storing
...
        output_file (str): The path to the file that will be written as the "QA
        template" file
...
        QA for.
        arc_characteristics (dict, default=None): A keyword argument which acts as a
        flag to determine if we should create the "arc_characteristics" data
        structure. If the input variable is not None, then this function is just used
        to write the QA template file. If it is None, then the data structure is
        created then the template file is written.
        max_num_when (int): An integer representing the maximum number of when
        conditions to store for different types of arcs
    Returns:
        arc_characteristics (dict): The dictionary which stores all of hte
        characteristics for the arcs we want to do MC QA for
    """
 
    if arc_characteristics is None:
        arc_characteristics, num_arcs_identified = getQAArcCharacteristics(
            template_info, valid_arc_types, cell_pattern_list, template_deck_type,
            max_num_when=max_num_when)
 
    write_list = list()
    for arc_count in arc_characteristics:
        # Ignore arcs that we don't know how to sensitize
        if arc_characteristics[arc_count]['template_deck'] is None:
            continue
 
        arc_info = getArcInfoLine(arc_characteristics[arc_count])
        slew_info = getSlewInfoLine(arc_characteristics[arc_count])
        template_deck_info = getTemplateDeckLine(
            arc_characteristics[arc_count])
        write_list.append(arc_info)
        write_list.append(slew_info)
        write_list.append(template_deck_info)
        write_list.append('')
 
    with open(output_file, 'w') as f:
        f.write('\n'.join(write_list))
 
    return arc_characteristics
 
 
...
 
    header_info = ' | '.join(["CELL %s" % cell_name,
                              "REL_PIN %s" % rel_pin,
...
    max_slew_str = getMaxSlew(rel_pin_slew_list, constr_pin_slew_list)
 
    slew_info = ' | '.join(["REL_PIN_SLEWS %s" % rel_pin_slew_str,
...
 
 
def getQAArcCharacteristics(template_info, valid_arc_types, cell_pattern_list,
                            template_type, chartcl_file, max_num_when=1):
    """
...
        if not checkValidCell(cell_name, cell_pattern_list) or cell_name not in template_info._tcl_vars['cells']:
            continue
 
...
                cell_name=cell_name, arc_type=arc_type, pin=pin, pin_dir=pin_dir,
                rel_pin=rel_pin, rel_pin_dir=rel_pin_dir, probe_list=probe_list,
                when=log_when, template_type=template_type)
 
            # Check counts
            if checkWhenCount(arc_type, pin, rel_pin, vector, lit_when, max_num_when,
...
                    if i == 0 or i == 4:
                        continue
                    arc = createArcInfo(cell_name, arc_type, pin, pin_dir, rel_pin,
                                        rel_pin_dir, lit_when, log_when, probe_list, cell_pinlist,
                                        output_pinlist, index_1.split(), index_2.split(), i,
                                        output_load[i], side_pin_state, template_deck, metric, metric_thresh, vector)
                    arc_list.append(arc)
            else:
                arc = createArcInfo(cell_name, arc_type, pin, pin_dir, rel_pin,
                                    rel_pin_dir, lit_when, log_when, probe_list, cell_pinlist,
                                    output_pinlist, index_1.split(), index_2.split(), None,
                                    output_load[0], side_pin_state, template_deck, metric, metric_thresh, vector)
                arc_list.append(arc)
...
 
 
def createArcInfo(cell_name, arc_type, pin, pin_dir, rel_pin, rel_pin_dir, when, log_when,
                  probe_list, cell_pinlist, output_pinlist, index_1, index_2, index_3_idx,
                  output_load, side_pin_state, template_deck, metric, metric_thresh, vector):
    """
...
        output_pinlist (list): A list (strings) of pins that correspond to the output
...
 
 
def checkWhenCount(arc_type, pin, rel_pin, vector, when, max_num_when, existing_arcs,
                   template_deck):
    """
...
        if (not when in existing_arcs[arc_tuple]) and (template_deck is not None):
...
 
 
def getCellOutputLoad(template_info, cell_obj, arc_obj, chartcl_file, load_index=2):
    """
...
    # Get the index2
    index_2 = ""
    if cell_obj.constraint() != None and re.search('5x5x5', cell_obj.constraint()):
...
    # Constraint
    if arc_type in ["hold", "removal", "non_seq_hold", "setup", "recovery",
                    "non_seq_setup", "nochange_low_low", "nochange_low_high",
                    "nochange_high_low", "nochange_high_high", "min_pulse_width"]:
        template_name = cell_obj.constraint()
        override_list = getOverrideList(cell_obj, "constraint")
 
    # MPW
    elif arc_type in ["mpw"]:
...
    valid_define_index_list = getMatchingDefineIndexBlocks(override_list, pin,
                                                           related_pin, when)
 
    # Get the best option
...
        if not pin in define_index.pin().split() and define_index.pin().strip() != '*':
            pin_match = False
 
...
        if override_type in x.type().split():
