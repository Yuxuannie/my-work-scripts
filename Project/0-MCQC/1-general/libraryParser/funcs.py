...
            if verbose:
                print("INFO:\t Current related pin is set to %s" % related_pin_name)
 
...
            if verbose:
                print("INFO:\t Current when condition is set to %s" % when_condition)
 
...
            if verbose:
                print("INFO:\t Current timing sense is set to %s" % timing_sense)
 
...
 
            if verbose:
                print("INFO:\t Current transition type is set to %s" % transition_type)
 
            # Check valid table
            valid_table = checkValidTable(cell_name, pin_name, related_pin_name,
                                          timing_type, transition_type)
            if not valid_table:
                if verbose:
                    print("INFO:\t Table on line %s is invalid. Skipping it." % i)
                continue
 
            # Store the data
            if verbose:
                printArcData(cell_name, pin_name, related_pin_name, timing_type,
                             timing_sense, when_condition, transition_type)
            i = storeTableData(lib_data, library_file, i, cell_name, pin_name,
                               related_pin_name, timing_type, timing_sense,
                               when_condition, transition_type, sigma_type_header,
                               index_header)
            num_tables_stored += 1
 
...
            print("INFO:\t If you specify char_type, it must be one of 'constraint' | 'delay' | 'slew'")
...
 
 
def printArcData(cell_name, pin_name, related_pin_name, timing_type, timing_sense,
...
 
 
def findSigmaType(library_file, starting_line, sigma_type_header,
                  index_header):
    """
...
 
 
def storeTableData(lib_data, library_file, table_line, cell_name, pin_name,
                   related_pin_name, timing_type, timing_sense, when_condition,
                   table_type, sigma_type_header, index_header):
    """
...
    addKeyToDict(lib_data[cell_name][pin_name][related_pin_name][timing_type], when_condition)
    addKeyToDict(lib_data[cell_name][pin_name][related_pin_name][timing_type][when_condition], timing_sense)
    addKeyToDict(lib_data[cell_name][pin_name][related_pin_name][timing_type][when_condition][timing_sense], table_type)
 
    # Get the sigma type, index_1, and index_2 data
    sigma_type, i = findSigmaType(library_file, table_line, sigma_type_header, index_header)
    index_1_data, index_2_data, i = getIndexValuesFromLines(library_file, i, index_header)
 
    # Store attributes
    lib_data[cell_name][pin_name][related_pin_name][timing_type][when_condition][timing_sense][table_type]['index_1'] = index_1_data
    lib_data[cell_name][pin_name][related_pin_name][timing_type][when_condition][timing_sense][table_type]['index_2'] = index_2_data
 
    # Get and store the timing values of the table
    timing_data, i = getTimingValuesFromTable(library_file, i)
    lib_data[cell_name][pin_name][related_pin_name][timing_type][when_condition][timing_sense][table_type][sigma_type] = timing_data
 
    return i
 
 
...
                        for tran in lib_data[cell][pin][rel_pin][timing_type][when]:
                            tran_data = lib_data[cell][pin][rel_pin][timing_type][when][tran]
 
...
        valid_timing_types = ["nochange_low_low", "nochange_low_high",
                              "nochange_high_low", "nochange_high_high"]
        valid_table_types = ["RISE_CONSTRAINT", "FALL_CONSTRAINT"]
 
    elif arc_type == "delay":
        valid_timing_types = ["clear", "preset",
                              "combinational",
...
                        for when in lib_info[in_cell][pin][rel_pin][timing_type][sense]:
                            for tran in lib_info[in_cell][pin][rel_pin][timing_type][sense][when]:
                                if not tran in valid_table_types:
                                    continue
 
                                table_info = lib_info[in_cell][pin][rel_pin][timing_type][sense][when][tran]
