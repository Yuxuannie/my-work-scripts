...
    # Required arguments
    required_arg_info = [
        ("--globals_file=", ":", "The path to the 'batchGlobals' file containing the "
                                "information required to create the SPICE decks."),
        ("", "", "\tFor examples of this file, see the 'batchGlobals.txt' file in the "
                 "'examples' directory of root path %s" % os.path.dirname(sys.argv[0]))
    ]
 
    # Optional arguments
    optional_arg_info = [
        ("--generate_all_combinations", ":", "Simultaneously generates the SPICE decks "
                                             "with the globals.txt file for each "
                                             "library in the batch."),
        ("--arc_csv_filter_file=", ":", "The path to the CSV file containing the "
...
        ("--estimate_cpus", ":", "A flag that will print resource estimation for "
                                 "simulating the library."),
        ("--spice_deck_format=", ":", "Determines the SPICE deck format that will be "
...
    print("\t>>> python %s "
          "\n\t\t--globals_file '/path_to_globals_file/batchGlobals.txt'" % sys.argv[0])
...
    output_path = os.path.join(root_output_path, lib_obj.lib_type, lib_obj.lgvt,
                               lib_obj.corner)
...
    write_buffer = list()
    output_inc_file = os.path.join(lib_obj.script_output_path, "%s.inc" % lib_obj.corner)
...
    lib_line_header, orig_lib_with_delm, lib_line_model_lib = lib_line.strip().split()
...
    lib_options['ROOT_NETLIST_PATH'] = os.path.join(user_options['KIT_PATH'],
                                                    lib_obj.lib_type, lib_obj.lgvt,
                                                    "Netlist", lib_obj.netlist_subdir)
 
...
    qa_template_file = os.path.join(os.path.dirname(lib_root_output_path),
                                    "%s_%s_%s.template.qa" % (lib_obj.lib_type,
                                                              lib_obj.lgvt,
                                                              lib_obj.corner))
...
    while True:
        lib_root_output_path = os.path.join(lib_obj.lib_output_path, "run%s" % count,
                                            "DECKS")
...
            continue
 
        write_buffer.append('set_var {0:<30} {1:<}'.format(var_name, lib_options[
            var_name.upper()]))
...
        user_options = globalsFileReader.readGlobalsFile(globals_file, user_options)
    else:
...
 
    # Get objects
    lib_objs_list = batchMode.getLVFKitInfo(user_options['KIT_PATH'], user_lib_combos)
 
 
    valid_lib_obj_list = batchMode.getValidLibGenerator(lib_objs_list)
    for lib_obj in valid_lib_obj_list:
        print("Creating directory structure, include file, and globals for library %s"
              % "\t{0:<10} {1:<10} {2:<50}".format(lib_obj.lib_type, lib_obj.lgvt,
                                                 lib_obj.corner))
...
    print("Find all data in the root path %s" % user_options['ROOT_OUTPUT_PATH'])
 
 
