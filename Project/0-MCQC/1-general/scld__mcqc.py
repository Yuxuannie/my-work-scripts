...
    # Required arguments
    required_arg_info = [
        ("--globals_file=", ":", "The path to the 'globals' file containing the "
...
        ("--output_path=", ":", "The root output path where the MC QC is generated."),
    ]
 
    # Optional arguments
    optional_arg_info = [
...
        print("Couldn't find the char template.tcl file for library %s %s %s" % (
            user_options['lib_type'], user_options['lgvt'], user_options['corner']))
...
    elif user_options['HT']:
        char_path = Path(kit_path) / lib_type / lgvt / 'HT' / 'Char'
    else:
...
    qa_template_file = os.path.join(user_options['output_path'], "%s_%s_%s.template.qa" %
...
    write_buffer = list()
    output_inc_file = os.path.join(user_options['output_path'], "%s.inc" %
                                   user_options['corner'])
...
            write_line = substituteLibLineWithUserModel(line,
                                                        user_options['USER_MODEL_FILE'])
...
        sys.exit("The %s variable was not set; cannot import module." % str(mke))
 
 
def main():
...
    user_options = globalsFileReader.readGlobalsFile(user_options['globals_file'],
                                                     user_options)
 
