...
    output_lpe_buffer = createOutputLPEBuffer(input_lpe_lines, xtor_list,
                                              parameter_list, output_lpe_header)
 
    # Write file
...
    p1 = subprocess.Popen(grep_cmd1, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    grep_cmd2 = ["bash", "-c", "grep '_mac'"]
    p2 = subprocess.Popen(grep_cmd2, stdin=p1.stdout, stdout=subprocess.PIPE,
                          stderr=subprocess.PIPE)
    # Grep out any commented-out MOS
    grep_cmd3 = ["bash", "-c", "grep -v '*'"]
    p3 = subprocess.Popen(grep_cmd3, stdin=p2.stdout, stdout=subprocess.PIPE,
                          stderr=subprocess.PIPE)
    stdout, stderr = p3.communicate(timeout=60)
    # Extract the XTORs
    xtor_list = list(set([x.split()[0] for x in stdout.decode().strip().split('\n')]))
    return xtor_list
 
 
...
        xtor_buff = list()
        _ = [xtor_buff.append("%s__%s=agauss(0,1,1)" % (xtor, parameter)) for parameter
                              in parameter_list]
...
 
 
def createOutputLPEBuffer(input_lpe_lines, xtor_list, parameter_list, output_lpe_header):
