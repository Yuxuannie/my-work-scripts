...
            unspecified_pins_lines = getUnspecifiedPinsLines(when_pins, arc_info)
...
       
        elif 'glitch__maxq' in arc_info['HEADER_INFO'] or 'glitch__1' in arc_info['HEADER_INFO']:
...
            pushout_per_line = template_line[:equals_idx+1]+' '+ metric_thresh+' '+template_line[pushout_idx:]
...
        metric_thresh = str(float("%.5f" % (1-float(arc_info['GLITCH']))))
        glitch_minq_line = template_line[:vertical_bar_idx+1]+' '+ metric_thresh + '\n'
...
            print ('{template_name}'.format(template_name=arc_info['TEMPLATE_DECK']))
            print ('---------------------------------')
            return template_line
        question_idx = template_line.index('?')
        metric_thresh = str(1-float(arc_info['GLITCH']))
        glitch_minq_line = template_line[:greater_idx+1]+' '+ metric_thresh+' '+template_line[question_idx:]
 
...
        glitch_minq_line = template_line[:apostrophe_idx+1]+' '+ metric_thresh + template_line[asterisk_idx:]
 
    return glitch_minq_line
 
def get_glitch_maxq_line(template_line, arc_info):
    glitch_maxq_line = template_line
    if '* MEAS_GLITCH_PER maxq' in template_line or '* MEAS_GLITCH_PER glitch_maxq' in template_line:
        vertical_bar_idx = template_line.index('|')
        metric_thresh = str(arc_info['GLITCH'])
        glitch_maxq_line = template_line[:vertical_bar_idx+1]+' '+ metric_thresh + '\n'
           
...
        glitch_maxq_line = template_line[:greater_idx+1]+' '+ metric_thresh+' '+template_line[question_idx:]
       
    elif '.meas tran maxq' in template_line and 'goal=' in template_line:
        apostrophe_idx = template_line.index('\'')
        asterisk_idx = template_line.index('*')
        metric_thresh = str(arc_info['GLITCH'])
        glitch_maxq_line = template_line[:apostrophe_idx+1]+' '+ metric_thresh + template_line[asterisk_idx:]
       
