...
class InputTraceabilityEngine:
    ...
    def get_cached_template_data(self, template_file_path: Path) -> Dict:
        ...
        if template_path_str not in self._template_cache:
            ...
            print("[OK] Template parsed and cached: {} arcs in {:.2f}s".format(len(template_data.get('arcs', [])), parse_time))
        else:
            if self.verbose:
    ...
    def _parse_template_file_complete(self, template_file: Path) -> Dict:
        ...
        try:
            ...
            self.logger.error("Error parsing template file {template_file}: {e}")
    ...
    def _extract_arc_attributes(self, full_command: str, line_start: int, line_end: int) -> Dict:
        ...
        if vector_match:
            attributes['vector'] = "{{{vector_match.group(1)}}}"
        ...
        if probe_match:
            attributes['probe'] = "{{{probe_match.group(1)}}}"
    ...
    def trace_arc_inputs(self, arc_folder: Path, template_file: Optional[Path] = None,
        ...
        self.logger.info("Tracing inputs for arc: {arc_name}")
        ...
        for globals_file_item in globals_files:
            globals_key = "globals_{globals_file_item.name}"
    ...
    def _find_template_file(self, arc_folder: Path) -> Optional[Path]:
        ...
        for candidate in candidates:
            if candidate.exists():
                self.logger.debug("Found template file: {candidate}")
                return candidate

        self.logger.warning("No template.tcl found for {arc_folder}")
        return None
    ...
    def _find_chartcl_file(self, arc_folder: Path) -> Optional[Path]:
        ...
        for candidate in candidates:
            if candidate.exists():
                self.logger.debug("Found chartcl file: {candidate}")
                return candidate

        self.logger.warning("No chartcl.tcl found for {arc_folder}")
        return None
        ...
        for search_dir in search_dirs:
            if search_dir.exists():
                for globals_file in search_dir.glob("*globals*.txt"):
                    ...
                    self.logger.debug("Found globals file: {globals_file}")
            ...
            with open(template_file, 'r') as f:
                for line_num, line in enumerate(f, 1):
                    ...
                    if any(keyword in line.lower() for keyword in [
                        ...
                        key_lines.append("Line {line_num}: {line}")

        except Exception as e:
            self.logger.error("Error reading template file {template_file}: {e}")
    ...
    def _extract_template_measurements(self, template_file: Path) -> List[Dict[str, str]]:
        ...
        try:
            ...
            self.logger.error("Error extracting measurements from {template_file}: {e}")
    ...
    def _extract_arc_specs(self, template_file: Path) -> Dict[str, List[str]]:
        ...
        try:
            ...
            self.logger.error("Error extracting arc specs from {template_file}: {e}")
    ...
    def _extract_key_chartcl_lines(self, chartcl_file: Path) -> List[str]:
        ...
        try:
            ...
            self.logger.error("Error reading chartcl file {chartcl_file}: {e}")
        ...
        try:
            ...
            if display_mode == 'minimal':
                    ...
                    for pattern in priority_patterns:
                        for var_name, var_info in all_vars.items():
                            ...
                            if pattern in var_lower:
                                relevant_vars[var_name] = {
                                    **var_info,
                                    'usage': "'Setup constraint setting',
                                    'relevance': 'Arc type + constraints'
                                }
                ...
                for var_name, var_info in all_vars.items():
                    ...
                    if any(pattern in var_lower for pattern in [
                        ...
                        relevant_vars[var_name] = {
                            **var_info,
                            'usage': "'Constraint-related setting',
            ...
            for var_name, var_info in all_vars.items():
                if cell_name.lower() in var_name.lower():
                    relevant_vars[var_name] = {
                        **var_info,
                        'usage': "'Cell-specific override for {cell_name}',
                        'relevance': "'Cell specific ({cell_name})'
                    }
            ...
            return {
                'relevant_vars': {},
                'error': "'Error parsing char.tcl file: {e}',
    ...
    def _extract_chartcl_measurements(self, chartcl_file: Path) -> List[Dict[str, str]]:
        ...
        try:
            ...
            self.logger.error("Error extracting measurements from {chartcl_file}: {e}")
    ...
    def _extract_chartcl_timing(self, chartcl_file: Path) -> Dict[str, List[str]]:
        ...
        try:
            ...
            self.logger.error("Error extracting timing config from {chartcl_file}: {e}")
            ...
            with open(globals_file, 'r') as f:
                for line_num, line in enumerate(f, 1):
                    ...
                    if line and not line.startswith('#'):
                        lines.append("Line {line_num}: {line}")

        except Exception as e:
            self.logger.error("Error reading globals file {globals_file}: {e}")
    ...
    def _extract_globals_parameters(self, globals_file: Path) -> Dict[str, str]:
        ...
        try:
            ...
            self.logger.error("Error extracting parameters from {globals_file}: {e}")
    ...
    def extract_template_from_deck(self, deck_file: Path) -> Dict:
        ...
        * TEMPLATE_DECK_PATH /path/to/template__CPN__rise__fall__1.sp
        ...
        try:
            ...
            return {
                'found': False,
                'error': "'Error reading deck file: {e}'
            }
    ...
    def _extract_tcl_variables(self, template_file: Path) -> Dict[str, str]:
        ...
        try:
            ...
            self.logger.error("Error extracting TCL variables from {template_file}: {e}")
    ...
    def _extract_when_conditions_from_template(self, template_file: Path) -> List[Dict[str, str]]:
        ...
        try:
            ...
            self.logger.error("Error extracting when conditions from {template_file}: {e}")
    ...
    def _extract_tcl_settings(self, chartcl_file: Path) -> Dict[str, str]:
        ...
        try:
            ...
            self.logger.error("Error extracting TCL settings from {chartcl_file}: {e}")
    ...
    def _extract_char_conditions(self, chartcl_file: Path) -> List[Dict[str, str]]:
        ...
        try:
            ...
            self.logger.error("Error extracting char conditions from {chartcl_file}: {e}")
    ...
    def _extract_spice_parameters(self, globals_file: Path) -> Dict[str, str]:
        ...
        try:
            ...
            self.logger.error("Error extracting SPICE parameters from {globals_file}: {e}")
    ...
    def _extract_model_information(self, globals_file: Path) -> Dict[str, Any]:
        ...
        try:
            with open(globals_file, 'r') as f:
                ...
                corner_keywords = ['tt', 'ss', 'f"', 's"', 'fs', 'typical', 'slow', 'fast']
                for keyword in corner_keywords:
                    if keyword in content.lower():
                        model_info['corners'].append(keyword)

        except Exception as e:
            self.logger.error("Error extracting model info from {globals_file}: {e}")
    ...
    def _analyze_mc_sim_outputs(self, mc_sim_file: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.error("Error analyzing mc_sim file {mc_sim_file}: {e}")
...
class SPICEDeckAnalyzer:
    ...
    def analyze_deck(self, deck_path: Path) -> Dict[str, Any]:
        ...
        if not deck_path.exists():
            return {
                'error': "Deck file not found: {deck_path}",
                'file_path': str(deck_path)
            }

        self.logger.info("Analyzing SPICE deck: {deck_path}")
        ...
        try:
            ...
            self.logger.error("Error analyzing deck {deck_path}: {e}")
    ...
    def detect_internal_node_measurement(self, deck_path: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.error("Error detecting internal nodes in {deck_path}: {e}")
...
class ComplianceValidator:
    ...
    def validate_compliance(self, traceability_data: Dict[str, Any], deck_analysis: Dict[str, Any], template_match_result=None) -> Dict[str, Any]:
        ...
        self.logger.info("Validating compliance for arc: {traceability_data.get('arc_name', 'unknown')}")
        ...
        try:
            ...
            if traceability_score >= self.thresholds['minimum_traceability']:
                ...
                test_result['issues'].append(
                    "Traceability score {traceability_score:.1%} below threshold {self.thresholds['minimum_traceability']:.1%}"
                )
            ...
            test_result['issues'].append("Traceability test error: {e}")
    ...
    def _test_specification_coverage(self, traceability_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if coverage_score >= self.thresholds['minimum_specification_coverage']:
                ...
                test_result['status'] = 'FAIL'
                test_result['issues'].append(
                    "Specification coverage {coverage_score:.1%} below threshold {self.thresholds['minimum_specification_coverage']:.1%}"
                )
            ...
            test_result['status'] = 'ERROR'
            test_result['issues'].append("Coverage test error: {e}")
    ...
    def _test_missing_specifications(self, traceability_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if critical_missing <= self.thresholds['maximum_missing_critical']:
                ...
                test_result['issues'].append(
                    "Critical missing specifications ({critical_missing}) exceeds threshold ({self.thresholds['maximum_missing_critical']})"
                )

            # Add warnings for high/medium missing specs
            if high_missing > 0:
                test_result['issues'].append("{high_missing} high-impact missing specifications detected")
            if medium_missing > 0:
                test_result['issues'].append("{medium_missing} medium-impact missing specifications detected")
            ...
            test_result['status'] = 'ERROR'
            test_result['issues'].append("Missing specifications test error: {e}")
    ...
    def _test_pattern_consistency(self, deck_analysis: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if inconsistency_count <= self.thresholds['maximum_inconsistencies']:
                ...
                test_result['score'] = 0.0
                test_result['issues'].append(
                    "Pattern inconsistencies ({inconsistency_count}) exceed threshold ({self.thresholds['maximum_inconsistencies']})"
                )
            ...
            test_result['status'] = 'ERROR'
            test_result['issues'].append("Pattern consistency test error: {e}")
    ...
    def _test_deck_structure(self, deck_analysis: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            if 'error' in deck_analysis:
                test_result['status'] = 'FAIL'
                test_result['score'] = 0.0
                test_result['issues'].append("Deck analysis error: {deck_analysis['error']}")
                return test_result
            ...
            if total_lines < 10:
                test_result['issues'].append("Deck too short: {total_lines} lines")
                structure_score -= 0.3
            ...
            test_result['status'] = 'ERROR'
            test_result['issues'].append("Deck structure test error: {e}")
    ...
    def _identify_critical_issues(self, tests: Dict[str, Dict]) -> List[str]:
        ...
        for test_name, test_result in tests.items():
            if test_result.get('status') == 'FAIL':
                ...
                for issue in issues:
                    critical_issues.append("{test_name}: {issue}")
...
class AlignmentAnalyzer:
    ...
    def generate_alignment_csv(self, alignment_results: List[Dict], output_file: Path):
        ...
        try:
            ...
            if self.verbose:
                print("Alignment CSV generated: {output_file}")

        except Exception as e:
            print("Error generating alignment CSV: {e}")
    ...
    def generate_statistics_report(self, alignment_results: List[Dict], output_file: Path, processing_stats: Dict):
        ...
        stats_content = """MCQC vs Template Validation Statistics
================================================================================
...
"""
        ...
        try:
            ...
            if self.verbose:
                print("Statistics report generated: {output_file}")

        except Exception as e:
            print("Error generating statistics report: {e}")
...
class ReportGenerator:
    ...
    def generate_structured_report(self, validation_data: Dict[str, Any], output_path: Path, display_mode: str = 'relevant') -> None:
        ...
        self.logger.info("Generating human-readable validation report: {output_path}")
        ...
        try:
            ...
            self.logger.info("Human-readable validation report written to: {txt_path}")

        except Exception as e:
            self.logger.error("Error writing validation report: {e}")
    ...
    def generate_csv_summary(self, validation_results: List[Dict[str, Any]], output_path: Path) -> None:
        ...
        self.logger.info("Generating CSV summary: {output_path}")
        ...
        try:
            with open(output_path, 'w', newline='') as f:
                ...
                for result in validation_results:
                    ...
                    writer.writerow([
                        ...
                        "{test_summary.get('pass_rate', 0.0):.1%}",
                        test_summary.get('passed_tests', 0),
                        test_summary.get('total_tests', 0),
                        "{test_summary.get('average_score', 0.0):.2f}",
            ...
            self.logger.info("CSV summary written to: {output_path}")

        except Exception as e:
            self.logger.error("Error writing CSV summary: {e}")
    ...
    def _format_test_results_section(self, validation_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for test_name, test_result in tests.items():
            section[test_name] = {
                ...
                'Score': "{test_result.get('score', 0.0):.2f}",
                'Issues': test_result.get('issues', []),
    ...
    def _format_detailed_data_section(self, validation_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for source_name, source_data in input_sources.items():
            if isinstance(source_data, dict):
                ...
                if file_path:
                    section['Input File Paths'].append("{source_name}: {file_path}")
    ...
    def _generate_arc_identification_section(self, arc_name: str, traceability_data: Dict, deck_analysis: Dict) -> List[str]:
        ...
        lines.extend([
            ...
            "  Cell Name:          {arc_info.get('cell_name', 'unknown')}",
            "  Arc Type:           {arc_info.get('arc_type', 'unknown')}",
            "  Constrained Pin:    {arc_info.get('constrained_pin', 'unknown')} ({arc_info.get('constrained_pin_dir', 'unknown')})",
            "  Related Pin:        {arc_info.get('related_pin', 'unknown')} ({arc_info.get('related_pin_dir', 'unknown')})",
            "  When Condition:     {arc_info.get('when_condition', 'unknown')}",
            "  Table Point:        {arc_info.get('table_point', 'unknown')}",
            "  Vector:             {arc_info.get('vector', 'unknown')}",
            "",
            "Parsing Metadata:",
            "  Parser Used:        Enhanced Arc Name Parser",
            "  Parsing Status:     {'SUCCESS' if arc_info.get('cell_name') != 'unknown' else 'PARTIAL'}",
            "  Raw Folder Name:    {arc_info.get('raw_folder_name', arc_name)}",
            ""
        ])
    ...
    def _generate_deck_analysis_section(self, deck_analysis: Dict) -> List[str]:
        ...
        lines.extend([
            "Analyzed File:      {deck_analysis.get('file_path', 'unknown')}",
            ""
        ])
    ...
    def _generate_compliance_validation_section(self, tests: Dict, overall_status: str, deck_analysis: Dict = None) -> List[str]:
        ...
        lines.extend([
            "Overall Status:     ? COMPLIANT WITH CURRENT SPECIFICATIONS",
            ""
        ])
        ...
        lines.extend([
            ...
            "Checks Passed:          {passed_checks} ({int(passed_checks/required_checks*100)}%)",
            "Checks Failed:          {failed_checks} ({int(failed_checks/required_checks*100)}%)",
            "Not Applicable:         3 (future specifications)",
            "",
            "Verdict:                {'? FULLY COMPLIANT' if failed_checks == 0 else '? NON-COMPLIANT'}",
            "Reason:                 {'Deck meets all current template.tcl specifications' if failed_checks == 0 else 'Missing required measurements'}",
    ...
    def _format_template_specifications(self, template_data: Dict, arc_info: Dict, display_mode: str = 'relevant') -> List[str]:
        ...
        lines.extend([
            "File:               {template_data.get('file_path', 'unknown')}",
            ""
        ])
        ...
        if template_file.exists():
            ...
            lines.extend([
                ...
                "  {'?' if result.cell_found else '?'} Cell Found:         {'YES' if result.cell_found else 'NO'} ({cell_name})",
                "  {'?' if result.arc_found else '?'} Arc Found:          {'YES' if result.arc_found else 'N/A' if not result.cell_found else 'NO'}",
                "  {'?' if result.success else '?'} Line Extraction:    {'SUCCESS' if result.success else 'FAILED'} {"'(Lines {result.line_start}-{result.line_end})' if result.success else ''}",
                ""
            ])
            ...
            if result.success:
                ...
                lines.extend([
                    "Match Details:",
                    "  ? Cell name:          {'MATCHED ?' if match_details.get('cell_name_match') else 'NOT MATCHED ?'}",
                    "  ? Arc type:           {'MATCHED ?' if match_details.get('arc_type_match') else 'NOT MATCHED ?'} ({arc_type})",
                    "  ? Related pin:        {'MATCHED ?' if match_details.get('related_pin_match') else 'NOT MATCHED ?'} ({related_pin})",
                    "  ? When condition:     {'MATCHED ?' if match_details.get('when_condition_match') else 'NOT MATCHED ?'} ({when_condition})",
                    "",
                ...
                for attr_name, attr_value in attrs.items():
                    if attr_name != 'match_reasons':  # Skip internal debugging
                        ...
                        line_info = "[Line {result.line_start}+]" if result.line_start else ""
                        lines.append("  {status} {attr_name:<18} \"{attr_value}\" {line_info}")
                ...
                if not result.cell_found:
                    lines.extend([
                        "  Searched in: {result.total_cells_searched} cells",
                        "  Suggestion: Check if cell name parsing from deck is correct"
                    ])
                elif not result.arc_found:
                    lines.extend([
                        "  Cell found but arc not matched",
                        "  Suggestion: Check arc type, related pin, or when condition"
                    ])
    ...
    def _format_chartcl_specifications(self, chartcl_data: Dict, arc_info: Dict, display_mode: str = 'relevant') -> List[str]:
        ...
        lines = self._create_subsection_divider("Chartcl.tcl Variables [Display Mode: {display_mode}]", "[B]")
        lines.extend([
            "File:               {chartcl_data.get('file_path', 'unknown')}",
            ""
        ])
        ...
        if chartcl_file.exists():
            ...
            if relevant_vars.get('relevant_vars'):
                for var_name, var_info in relevant_vars['relevant_vars'].items():
                    ...
                    lines.append("{var_name:<25} {value:<15} [Line {line_num}]")
                ...
                if suppressed > 0:
                    if display_mode == 'minimal':
                        lines.append("({suppressed} other variables not shown. Use --chartcl-display=relevant to show more)")
                    elif display_mode == 'relevant':
                        lines.append("({suppressed} other variables not shown. Use --chartcl-display=all to show all)")
                    lines.append("")

            else:
                lines.extend([
                    "[ERROR] No relevant variables found or error parsing char.tcl",
                    "   Error: {relevant_vars.get('error', 'Unknown error')}",
                    ""
                ])
        else:
            lines.extend([
                "[ERROR] Chartcl file not found or inaccessible",
                ""
            ])
    ...
    def _format_globals_specifications(self, globals_sources: Dict) -> List[str]:
        ...
        for source_name, globals_data in globals_sources.items():
            lines.append("File:               {globals_data.get('file_path', 'unknown')}")
            lines.append("")
            ...
            if spice_params:
                ...
                for param_name, param_value in spice_params.items():
                    lines.append("  ? {param_name:20} {param_value}")
                lines.append("")
    ...
    def _format_template_selection(self, deck_analysis: Dict) -> List[str]:
        ...
        if deck_file_path:
            ...
            if template_info.get('found'):
                lines.extend([
                    "Selected Template:  {template_info['filename']}",
                    "Source:             Extracted from deck header (Line {template_info['found_at_line']})",
                    "Full Path:          {template_info['full_path']}",
                    "",
                    "Template Selection Logic:",
                    "  Status:           ? Explicitly specified in generated deck",
                    "  Method:           Parsed from TEMPLATE_DECK_PATH header",
                    "  Raw Line:         {template_info.get('raw_line', 'N/A')}",
                    ""
                ])
            else:
                lines.extend([
                    "Selected Template:  [ERROR] NOT FOUND in deck header",
                    "Error:              {template_info.get('error', 'Unknown error')}",
                    "",
                    "Template Selection Logic:",
            ...
            lines.extend([
                "Selected Template:  [ERROR] DECK FILE PATH NOT AVAILABLE",
                "",
                "Template Selection Logic:",
    ...
    def _format_primary_measurements(self, deck_analysis: Dict) -> List[str]:
        ...
        self.logger.debug("Searching for cp2q_del1 in {len(measurements)} measurements")
        for i, meas in enumerate(measurements):
            meas_name = meas.get('measurement_name', 'unknown')
            self.logger.debug("  Measurement {}: '{}'".format(i+1, meas_name))
            if 'cp2q_del1' in meas_name:
                ...
                lines.extend([
                    "cp2q_del1           ? PRESENT (Line {line_num})",
                    "  Type:             Primary delay measurement (required for all MPW arcs)",
                    "  Content:          {full_line[:80]}{'...' if len(full_line) > 80 else ''}",
                    "",
                    "  Analysis:",
                    "    Trigger:        Pin transition detection",
                    "    Target:         Output response measurement",
                    "    Purpose:        Core timing characterization",
                    "    Status:         ? PASS - Required measurement present",
                    ""
                ])
                break
    ...
    def _format_final_state_analysis(self, deck_analysis: Dict) -> List[str]:
        ...
        for meas in measurements:
            ...
            if 'final_state' in meas_name:
                ...
                lines.extend([
                    "final_state         ? PRESENT (Line {line_num})",
                    "  Type:             Optional end-state verification",
                    "  Content:          {full_line[:80]}{'...' if len(full_line) > 80 else ''}",
                    "  Status:           Present in this deck",
                    "  Template Spec:    ? Not specified in template.tcl (added by Python logic)",
                    "  Note:             Found in 74% of other arcs",
                    ""
                ])
            ...
            if 'final_state_check' in meas_name:
                ...
                line_num = meas.get('line_number', 'unknown')
                lines.extend([
                    "final_state_check   ? PRESENT (Line {line_num})",
                    "  Type:             Pass/fail criteria for final-state",
                    "  Status:           Active verification criteria",
                    ""
                ])
    ...
    def _format_monitoring_cycles_analysis(self, deck_analysis: Dict) -> List[str]:
        ...
        for meas in measurements:
            ...
            if 'cp2q_del2' in meas_name:
                ...
                lines.extend([
                    "cp2q_del2           ? PRESENT (Line {line_num})",
    ...
    def _format_measurement_nodes_analysis(self, deck_analysis: Dict) -> List[str]:
        ...
        if internal_nodes:
            lines.extend([
                ...
                "  Status:           ? Using internal node measurement",
                "  Type:             Internal node (not direct output)",
                "  Note:             Some arcs use internal nodes (e.g., X1.Q1)",
                "                    Pattern for internal node selection is unclear",
                "  Future:           Will be specified via: measurement_node: \"{primary_node}\"",
                ""
            ])
            ...
            lines.extend([
                "Primary Node:       {primary_node} (output pin)",
                "  Status:           ? Standard output pin measurement",
                "  Type:             Direct output (not internal node)",
                "  Note:             Some arcs use internal nodes (e.g., X1.Q1)",
                "                    Pattern for internal node selection is unclear",
                "  Future:           Will be specified via: measurement_node: \"Q\" or \"X1.Q1\"",
                ""
            ])
    ...
    def _calculate_test_summary(self, tests: Dict) -> Dict[str, Any]:
        ...
        return {
            ...
            'found_count': "{passed_tests} of {total_tests} basic measurements",
    ...
    def _format_compliance_assessment(self, tests: Dict) -> List[str]:
        ...
        for test_name, test_result in tests.items():
            ...
            lines.append("  {icon:8} {display_name} (score: {score:.2f})")
...
def find_arcs_to_process(deck_dir: Path, output_dir: Path, force: bool = False):
    ...
    if force:
        print("--force flag set: will reprocess all {len(all_arc_paths)} arcs")
    ...
    for arc_folder in all_arc_paths:
        ...
        report_file = arc_folder / "{arc_name}_alignment_report.txt"
    ...
    print("Found {len(all_arc_paths)} total arcs")
    print("  Already processed: {len(arcs_skipped)} arcs (will skip)")
    print("  Need processing: {len(arcs_to_process)} arcs")
...
def generate_csv_from_existing_reports(report_dir: Path, output_csv: Path):
    ...
    """
    print("Scanning existing reports in {report_dir}")

    report_files = list(report_dir.glob("*_alignment_report.txt"))
    print("Found {len(report_files)} existing reports")
    ...
    for report_file in report_files:
        try:
            ...
            print("Error parsing {report_file}: {e}")
    ...
    if rows:
        ...
        print("Generated CSV: {output_csv} ({len(rows)} arcs)")
    else:
...
def parse_alignment_report_file(report_file: Path) -> Dict:
    ...
    try:
        ...
        print("Error parsing report file {report_file}: {e}")
        return None
...
"""
    ...
    if template_match and template_match.success:
        content += """? Found in template.tcl (Lines {template_match.line_start}-{template_match.line_end})
...
"""
    ...
    try:
        ...
        print("Error writing simplified report to {output_file}: {e}")
...
def process_single_arc(arc_data):
    ...
    try:
        ...
        if not mc_sim_file.exists():
            print("[ERROR] {arc_name}: mc_sim.sp not found")
        ...
        if not args.csv_only:
            arc_report_file = arc_folder / "{arc_name}_alignment_report.txt"
        ...
        if verbose and total_time > 2.0:  # Only show timing breakdown for slow arcs
            print("  [TIME]  Timing breakdown: {total_time:.2f}s total")
            print("     Template: {template_time:.2f}s, Deck: {deck_time:.2f}s, Validation: {validation_time:.2f}s")

        return (arc_folder, validation_data)

    except Exception as e:
        print("[ERROR] Error processing {arc_name}: {str(e)}")
        if verbose:
            import traceback
            traceback.print_exc()
        return (arc_folder, None)
...
Examples:
  ...
  python audit_deck_compliance.py --deck_dir /work/DECKS/ \\
    ...
    parser.add_argument(
        ...
        help="'Number of parallel workers (default: {min(cpu_count(), 8)}, detected: {cpu_count()} cores)'
    )
    parser.add_argument(
        '--force',
        action='store_true',
    ...
    if args.arc_folder:
        if args.arc_folder.exists():
            ...
            print("[ERROR] Arc folder not found: {args.arc_folder}")
            return 1
        ...
        if args.deck_dir.exists():
            ...
            if not arc_folders:
                print("[ERROR] No arc folders with mc_sim.sp found in: {args.deck_dir}")
                return 1
        else:
            print("[ERROR] Deck directory not found: {args.deck_dir}")
            return 1
    ...
    if args.template_file and args.template_file.exists():
        print("[CONFIG] Pre-parsing template for shared cache: {args.template_file}")
        ...
        try:
            ...
            print("   [OK] Template cached: {shared_template_data.get('total_arcs', 0)} arcs, {shared_template_data.get('total_cells', 0)} cells")
        except Exception as e:
            print("   [WARN]  Template caching failed: {e}")
            shared_template_data = None
    ...
    if not args.force and args.deck_dir:
        ...
        if skipped_arcs:
            print("   [SKIP]  Skipped {len(skipped_arcs)} arcs with existing reports")

    print("[SEARCH] Processing {len(arc_folders)} arc folders...")
    ...
    if len(arc_folders) > 1 and args.parallel > 1:
        ...
        print("[PERF] Processing {total_arcs} arcs using {workers} parallel workers...")
        if args.verbose:
            print("   [DATA] Performance optimizations active:")
            print("      ? Template caching: Parse once, use {total_arcs} times")
            print("      ? Parallel processing: {workers} cores")
            print("      ? Deck optimization: Stream processing")
            print("   [ETA] Target: <30 minutes total (vs {total_arcs * 60:.0f}s original)")
        print()
        ...
        print("     [CLOCK] Total time:          {parallel_time:.1f}s")
        print("     [SPEED] Avg per arc:         {avg_time_per_arc:.2f}s")
        print("     [PERF] Actual speedup:      {actual_speedup:.0f}x vs original")
        ...
        for i, (arc_folder, validation_data) in enumerate(results, 1):
            if validation_data:
                ...
                if args.verbose or i % max(1, total_arcs // 10) == 0 or i <= 3 or i >= total_arcs - 1:
                    print("  [{i:3d}/{total_arcs}] {status_icon} {arc_folder.name}: {status}")
            else:
                error_count += 1
                # Create error result
                error_result = {
                ...
                print("  [{i:3d}/{total_arcs}] [ERROR] {arc_folder.name}: ERROR")

        # Summary stats
        print("\n[STATS] Parallel Processing Summary:")
        print("   [OK] PASS:  {pass_count:3d} ({pass_count/total_arcs*100:.1f}%)")
        print("   [WARN]  FAIL:  {fail_count:3d} ({fail_count/total_arcs*100:.1f}%)")
        print("   [ERROR] ERROR: {error_count:3d} ({error_count/total_arcs*100:.1f}%)")
        print("   [SPEED] Speedup: {actual_speedup:.0f}x vs sequential (Target: 100x+)")

    else:
        # Sequential processing for single arc or when parallel disabled
        total_arcs = len(arc_folders)
        print("[PARSING] Processing {total_arcs} arcs sequentially...")
        if total_arcs > 1:
            print("   [TIP] Tip: Use --parallel {min(cpu_count(), 8)} for {min(cpu_count(), 8)}x speedup")
        if args.verbose:
            print("   [DATA] Performance optimizations active:")
        ...
        for i, (arc_folder, args_copy, verbose, shared_template_data) in enumerate(arc_data_list, 1):
            arc_start_time = time.time()
            print("[{i:3d}/{total_arcs}] Processing: {arc_folder.name}")
            ...
            if validation_data:
                ...
                print("     {status_icon} {status} ({arc_time:.1f}s)")
                if total_arcs > 10 and i % 5 == 0:  # Show ETA every 5 arcs for longer runs
                    print("     [ETA] Progress: {i/total_arcs*100:.1f}% | ETA: {remaining_time:.0f}s")
            else:
                error_count += 1
                # Create error result
                error_result = {
                ...
                validation_results.append(error_result)
                print("     [ERROR] ERROR ({arc_time:.1f}s)")
        ...
        print("\n[STATS] Sequential Processing Summary:")
        print("   [OK] PASS:  {pass_count:3d} ({pass_count/total_arcs*100:.1f}%)")
        print("   [WARN]  FAIL:  {fail_count:3d} ({fail_count/total_arcs*100:.1f}%)")
        print("   [ERROR] ERROR: {error_count:3d} ({error_count/total_arcs*100:.1f}%)")
        print("   [CLOCK] Total: {sequential_time:.1f}s ({sequential_time/total_arcs:.1f}s per arc)")
        print("   [SPEED] Speedup: {actual_speedup:.0f}x vs original (Target: 100x+)")
    ...
    if not args.force and args.deck_dir and alignment_results:
        ...
        print("   [PARSING] Including {len(alignment_results)} newly processed arcs")
        ...
        print("   [FILES] Scanning for existing reports in arc directories...")
        existing_alignment_data = []
        for arc_path in all_arc_paths:
            arc_name = arc_path.name
            existing_report = arc_path / "{arc_name}_alignment_report.txt"
            if existing_report.exists() and arc_path not in [arc for arc, _, _, _ in arc_data_list]:
                try:
                    ...
                    print("   [WARN]  Error parsing {existing_report}: {e}")

        if existing_alignment_data:
            print("   [OK] Found {len(existing_alignment_data)} existing reports to include")
            ...
            print("   [DATA] Generated comprehensive CSV: {len(combined_alignment_data)} total arcs")
        else:
    ...
    print("   Total arcs processed: {len(validation_results)}")

    statuses = [result.get('overall_status', 'UNKNOWN') for result in validation_results]
    print("   PASS: {statuses.count('PASS')}")
    print("   FAIL: {statuses.count('FAIL')}")
    print("   ERROR: {statuses.count('ERROR')}")

    pass_rate = statuses.count('PASS') / len(statuses) if statuses else 0.0
    print("   Overall pass rate: {pass_rate:.1%}")
    ...
    if total_with_template > 0:
        ...
        print("   ? Successfully matched: {template_successes} arcs ({template_success_rate:.1%})")
        print("   ? Cell not found:       {cell_not_found} arcs ({cell_not_found/total_with_template:.1%})")
        print("   ? Arc not found:        {arc_not_found} arcs ({arc_not_found/total_with_template:.1%})")
    ...
    if len(validation_results) > 10:  # Only show performance metrics for larger runs
        ...
        print("   [TIME]  Total processing time: {total_processing_time:.1f}s ({total_processing_time/60:.1f} minutes)")
        print("   [SPEED] Average per arc: {total_processing_time/len(validation_results):.2f}s")
        ...
        print("   [TARGET] Actual speedup achieved: {actual_speedup:.0f}x vs original (target: 100x+)")
        if shared_template_data:
            print("   [OK] Template caching: WORKING (parsed once, used {len(validation_results)} times)")
        else:
            print("   [WARN]  Template caching: NOT USED (no template file provided)")

    print("\n[FILES] Summary reports in: {args.output_dir}")
    print("   [DATA] PRIMARY: {alignment_csv_file}")
    print("   [STATS] Statistics: {stats_file}")
    print("   [DATA] Legacy CSV: {csv_file}")
    print("   [TARGET] Template matching: {template_summary_file}")

    if not args.csv_only:
        print("   ? Individual reports: [arc_name]_alignment_report.txt in each arc directory")

    print("\n[OK] MCQC validation completed successfully!")
    if total_processing_time < 30 * 60:  # Less than 30 minutes
        print("[SUCCESS] Performance target achieved: {total_processing_time:.1f}s < 30 minutes")
    else:
        print("[WARN]  Performance target missed: {total_processing_time/60:.1f} minutes > 30 minutes")
        print("   [STATS] Processed {len(validation_results)} arc directories")
