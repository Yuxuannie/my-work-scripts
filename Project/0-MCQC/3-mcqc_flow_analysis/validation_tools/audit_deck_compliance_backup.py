...
def process_single_arc(arc_data):
    ...
    try:
        ...
        if not mc_sim_file.exists():
            print(f"[ERROR] {arc_name}: mc_sim.sp not found")
  ...
  python audit_deck_compliance.py --deck_dir /work/DECKS/ \\
    ...
    if args.arc_folder:
        if args.arc_folder.exists():
            ...
            print(f"[ERROR] Arc folder not found: {args.arc_folder}")
            return 1
        ...
        if args.deck_dir.exists():
            ...
            if not arc_folders:
                print(f"[ERROR] No arc folders with mc_sim.sp found in: {args.deck_dir}")
                return 1
        else:
            print(f"[ERROR] Deck directory not found: {args.deck_dir}")
            return 1
    ...
    if args.template_file and args.template_file.exists():
        print(f"[CONFIG] Pre-parsing template for shared cache: {args.template_file}")
        ...
        try:
            ...
            print(f"   [OK] Template cached: {shared_template_data.get('total_arcs', 0)} arcs, {shared_template_data.get('total_cells', 0)} cells")
        except Exception as e:
            print(f"   [WARN]  Template caching failed: {e}")
            shared_template_data = None

    # Filter arcs based on --force flag
    if not args.force and args.deck_dir:
        print(f"[SEARCH] Checking for existing reports (use --force to reprocess all)...")
        arc_folders, skipped_arcs = find_arcs_to_process(args.deck_dir, args.output_dir, force=args.force)
        if skipped_arcs:
            print(f"   [SKIP]  Skipped {len(skipped_arcs)} arcs with existing reports")

    print(f"[SEARCH] Processing {len(arc_folders)} arc folders...")
    ...
    if len(arc_folders) > 1 and args.parallel > 1:
        ...
        print(f"[PERF] Processing {total_arcs} arcs using {workers} parallel workers...")
        if args.verbose:
            print(f"   [DATA] Performance optimizations active:")
            ...
            print(f"   [ETA] Target: <30 minutes total (vs {total_arcs * 60:.0f}s original)")
        print()
        ...
        print(f"  [SPEED] Parallel processing completed!")
        print(f"     [CLOCK] Total time:          {parallel_time:.1f}s")
        print(f"     [SPEED] Avg per arc:         {avg_time_per_arc:.2f}s")
        print(f"     [PERF] Actual speedup:      {actual_speedup:.0f}x vs original")
        print(f"     [MEMORY] Memory efficiency:   Stream processing enabled")
        print()
        ...
        for i, (arc_folder, validation_data) in enumerate(results, 1):
            if validation_data:
                ...
                print(f"  [{i:3d}/{total_arcs}] [ERROR] {arc_folder.name}: ERROR")

        # Summary stats
        print(f"\n[STATS] Parallel Processing Summary:")
        print(f"   [OK] PASS:  {pass_count:3d} ({pass_count/total_arcs*100:.1f}%)")
        print(f"   [WARN]  FAIL:  {fail_count:3d} ({fail_count/total_arcs*100:.1f}%)")
        print(f"   [ERROR] ERROR: {error_count:3d} ({error_count/total_arcs*100:.1f}%)")
        print(f"   [SPEED] Speedup: {actual_speedup:.0f}x vs sequential (Target: 100x+)")

    else:
        # Sequential processing for single arc or when parallel disabled
        total_arcs = len(arc_folders)
        print(f"[PARSING] Processing {total_arcs} arcs sequentially...")
        if total_arcs > 1:
            print(f"   [TIP] Tip: Use --parallel {min(cpu_count(), 8)} for {min(cpu_count(), 8)}x speedup")
        if args.verbose:
            print(f"   [DATA] Performance optimizations active:")
        ...
        for i, (arc_folder, args_copy, verbose, shared_template_data) in enumerate(arc_data_list, 1):
            ...
            if validation_data:
                ...
                if total_arcs > 10 and i % 5 == 0:  # Show ETA every 5 arcs for longer runs
                    print(f"     [ETA] Progress: {i/total_arcs*100:.1f}% | ETA: {remaining_time:.0f}s")
            else:
                error_count += 1
                # Create error result
                error_result = {
                ...
                validation_results.append(error_result)
                print(f"     [ERROR] ERROR ({arc_time:.1f}s)")
        ...
        print(f"   [WARN]  FAIL:  {fail_count:3d} ({fail_count/total_arcs*100:.1f}%)")
        print(f"   [ERROR] ERROR: {error_count:3d} ({error_count/total_arcs*100:.1f}%)")
        print(f"   [CLOCK] Total: {sequential_time:.1f}s ({sequential_time/total_arcs:.1f}s per arc)")
        print(f"   [SPEED] Speedup: {actual_speedup:.0f}x vs original (Target: 100x+)")
    ...
    if not args.force and args.deck_dir and alignment_results:
        ...
        print(f"   [PARSING] Including {len(alignment_results)} newly processed arcs")
        ...
        print(f"   [FILES] Scanning for existing reports in arc directories...")
        ...
        for arc_path in all_arc_paths:
            ...
            if existing_report.exists() and arc_path not in [arc for arc, _, _, _ in arc_data_list]:
                try:
                    ...
                    print(f"   [WARN]  Error parsing {existing_report}: {e}")

        if existing_alignment_data:
            print(f"   [OK] Found {len(existing_alignment_data)} existing reports to include")
            ...
            print(f"   [DATA] Generated comprehensive CSV: {len(combined_alignment_data)} total arcs")
        else:
    ...
    if len(validation_results) > 10:  # Only show performance metrics for larger runs
        ...
        print(f"   [TIME]  Total processing time: {total_processing_time:.1f}s ({total_processing_time/60:.1f} minutes)")
        print(f"   [SPEED] Average per arc: {total_processing_time/len(validation_results):.2f}s")
        ...
        print(f"   [TARGET] Actual speedup achieved: {actual_speedup:.0f}x vs original (target: 100x+)")
        if shared_template_data:
            print(f"   [OK] Template caching: WORKING (parsed once, used {len(validation_results)} times)")
        else:
            print(f"   [WARN]  Template caching: NOT USED (no template file provided)")

    print(f"\n[FILES] Summary reports in: {args.output_dir}")
    print(f"   [DATA] PRIMARY: {alignment_csv_file}")
    print(f"   [STATS] Statistics: {stats_file}")
    print(f"   [DATA] Legacy CSV: {csv_file}")
    print(f"   [TARGET] Template matching: {template_summary_file}")
    ...
    if total_processing_time < 30 * 60:  # Less than 30 minutes
        print(f"[SUCCESS] Performance target achieved: {total_processing_time:.1f}s < 30 minutes")
    else:
        print(f"[WARN]  Performance target missed: {total_processing_time/60:.1f} minutes > 30 minutes")
        print(f"   [STATS] Processed {len(validation_results)} arc directories")
