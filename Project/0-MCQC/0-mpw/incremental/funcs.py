...
    missing_sample_indices = getMissingSampleIndices(
        nsamples, simulated_sample_indices
    )
...
    else:
        print(
            "%s samples still need to be simulated." %
            len(missing_sample_indices)
        )
...
        shutil.copyfile(
            os.path.join(sim_path, "mc_sim.sp"),
            os.path.join(backup_folder, "mc_sim.sp")
        )
 
        # Create the incremental SPICE deck
        print("BEGIN PHASE 3.")
        print("Generating incremental deck buffer.")
        inc_buff = getIncrementalMCDeckBuffer(
            sim_path, missing_sample_indices, nsamples
        )
...
        recol_worker_mt0_files.append(
            os.path.join(sim_path, "incremental_mc_sim.mt0")
        )
...
                    ','.join([str(x) for x in missing_sample_indices])
                )
...
            sampling_method = line.split("sampling_method="
                                        )[1].strip().split()[0]
            break
 
...
    else:
        existing_sim_run_cnts = [
            int(os.path.basename(x).split('__run__')[1]) for x in existing_dirs
        ]
...
        if blacklisted:
            print(
                "Skipping item %s because it is in the backup blacklist." %
                content
            )
        else:
...
    backup_blacklist = ["mc_sim.sp", "sim__run__*", "nominal_sim*"]
 
...
    # ./*_dp/*/*.mt0
    _ = [
        worker_mt0_files.append(x)
        for x in glob.glob(os.path.join(sim_path, "*_dp", "*", "*.mt0"))
    ]
 
    # ./*.mt0
    _ = [
        worker_mt0_files.append(x)
        for x in glob.glob(os.path.join(sim_path, "*.mt0"))
    ]
 
    # ./sim__run__*/*_dp/*/*.mt0
    _ = [
        worker_mt0_files.append(x) for x in
        glob.glob(os.path.join(sim_path, "sim__run__*", "*_dp", "*", "*.mt0"))
    ]
 
    # ./sim__run__*/*.mt0
    _ = [
        worker_mt0_files.append(x)
        for x in glob.glob(os.path.join(sim_path, "sim__run__*", "*.mt0"))
    ]
 
...
        _ = [
            simulated_sample_indices.append(int(x.strip().split()[index_col]))
            for x in mt0_lines[(line_idx + 1):]
        ]
 
