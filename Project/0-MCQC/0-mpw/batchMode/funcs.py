...
 
 
def pygrep(in_file, pattern, count=100, proc_in=None):
    sys_cmd = ["bash", "-c", "grep -m %s '%s' %s" % (count, pattern, in_file)]
    if proc_in is None:
        proc = subprocess.Popen(
            sys_cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE
        )
...
    """
    valid_lib_gen = (
        lib_obj for lib_obj in lib_objs_list if lib_obj.valid_lib is True
    )
    return valid_lib_gen
 
 
def getUserLibraryCombinations(
    user_lib_file, user_star_file, user_exceptions_file
):
...
        line_combinations = line_parser.parseLine(
            input_line.strip(), user_stardict, user_exceptions
        )
 
...
    netlist_subdir = os.path.dirname(
        stdout.decode().strip().split('Netlist/')[1]
    )
...
            os.path.join(
                lib_obj.kit_path, lib_obj.lib_type, lib_obj.lgvt, "Template",
                "*%s*.template.tcl" % lib_obj.corner
            )
        )[0]
    except IndexError as _:
        print(
            "Couldn't find a template file for library %s %s %s" %
...
        char_file = glob.glob(
            os.path.join(
                lib_obj.kit_path, lib_obj.lib_type, lib_obj.lgvt, "Char",
                "char_%s.tcl" % lib_obj.corner
            )
        )[0]
    except IndexError as _:
        print(
            "Couldn't find a char file for library %s %s %s" %
            (lib_obj.lib_type, lib_obj.lgvt, lib_obj.corner)
        )
        print("Marking this library as invalid.")
        lib_obj.valid_lib = False
...
            os.path.join(
                lib_obj.kit_path, lib_obj.lib_type, lib_obj.lgvt, "Char",
                "%s.inc" % lib_obj.corner
            )
        )[0]
    except IndexError as _:
        print(
            "Couldn't find an include file for library %s %s %s" %
            (lib_obj.lib_type, lib_obj.lgvt, lib_obj.corner)
        )
        print("Marking this library as invalid.")
        lib_obj.valid_lib = False
...
    lib_voltage = '_'.join(
        ['p'.join(('%sv' % x).split('.')) for x in raw_voltages]
    )
