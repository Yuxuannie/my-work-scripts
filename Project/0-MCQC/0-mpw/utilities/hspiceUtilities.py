...
        return (float(self.perc_value) -
                float(self.nominal_value)) / float(divisor)
 
...
            [
                x.strip()
                for x in stdout.decode().strip().split('Time:')[1].split()
            ]
        )
 
        # Compute
...
    grep_cmd = [
        "bash", "-c",
        "grep 'Number of workers requested' %s" % progress_file
    ]
    proc = subprocess.Popen(
        grep_cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE
    )
...
    try:
        runtime_sec_f = float(
            stdout.decode().strip().split(' seconds')[0].split()[-1]
        )
        runtime_hrs_f = runtime_sec_f / 3600
    except IndexError as _:
        runtime_sec_f = None
...
    stdout, stderr = proc.communicate()
    try:
        threads_reqstd_f = float(
            stdout.decode().strip().split('-mt ')[1].split()[0]
        )
    except IndexError as _:
...
 
    # Get percentile
    percentile_value = getPercentileValueFromMeasurements(
        mt0_obj.data_values, mt0_obj.percentile
    )
...
    percentile_column_header = getPercentileFromMPP0File(
        mpp0_data, mpp0_lines, param_name, percentile_header
    )
 
...
    try:
        nsample_line_idx = fileIO.findMatchesInBuffer(
            mpp0_lines, "Number of Samples:*"
        )[0]
...
    percentile_line_idx = fileIO.findMatchesInBuffer(
        mpp0_lines, "%s *" % param_name, percentile_header_line_idx
    )[0]
 
...
 
    # Store
    storeLBPercentileMPP0(
        mpp0_data, lb_line, percentile_idx, percentile_header_line
    )
    storeUBPercentileMPP0(
        mpp0_data, ub_line, percentile_idx, percentile_header_line
    )
    storePredPercentileMPP0(
        mpp0_data, pred_line, percentile_idx, percentile_header_line
    )
 
...
    # Parse the header
    idx_dict, sample_moments_line_idx = getSampleMomentsIndicesFromMPP0File(
        mpp0_lines
    )
 
    # Get the pred value line index
    param_moments_line_idx = fileIO.findMatchesInBuffer(
        mpp0_lines, "%s *" % param_name, sample_moments_line_idx
    )[0]
 
    # Get the actual lines
...
            mpp0_lines, "*CI*mean*median*stdDev*mad*skewness*kurtosis*"
        )[0]
 
...
                percentile_index = mpp0_lines[starting_line].strip().split(
                ).index(x)
                percentile_column_header = mpp0_lines[starting_line].strip(
                ).split()[percentile_index]
                break
 
    except IndexError as _:
...
        (
            "--param_name=", ":",
            "The parameter name whose values should be extracted."
        )
    ]
 
    optional_arg_info = [
...
    print("\nFunctionality:")
    print(
        "A simple MPP0 parser which prints out the following data:"
        "\n\t Mean"
        "\n\t Median"
        "\n\t Skewness"
        "\n\t Kurtosis"
...
    print("\nNOTE:")
    print(
        "The user should specify as many digits as possible for the percentile "
        "header."
        "\n\tInternally, the script searches for a wildcard match so specifying 'Q99' "
        "to extract 'Q99.865' may result in unwanted data."
    )
    print(
        "To help debug any issues, the column header of the extracted percentile is "
        "printed."
    )
 
    print("\nUsage examples:")
    print(
        "\tParse the MPP0 file searching for param name 'cp2d' and percentile 'Q99'"
    )
    print(
        "\t>>> python %s "
...
    shortopts = "h"
    longopts = ["help", "mpp0_file=", "param_name=", "percentile_header="]
 
...
    mpp0_data, percentile_column_header = parseMPP0File(
        user_options['mpp0_file'], user_options['param_name'],
