...
 
 
def parseQACharacteristicsInfo(
    arc_list,
    root_netlist_path,
    index_1_index,
    index_2_index,
...
        mpw_input_threshold = calculate_mpw_input_threshold(chartcl_vars)
        spice_deck_info[arc_count]['MPW_INPUT_THRESHOLD'] = mpw_input_threshold
 
        template_deck = arcCharacteristics__getTemplateDeck(arc)
        spice_deck_info[arc_count]['TEMPLATE_DECK'] = template_deck
 
        header_info = arcCharacteristics__getHeaderInfo(
            arc, mpw_input_threshold)
...
        arcCharacteristics__storeProbePins(
            arc, spice_deck_info[arc_count], output_pins
        )
 
...
 
        max_slew = arcCharacteristics__getMaxSlew(template_info)
...
        spice_deck_info[arc_count]['GLITCH'] = chartcl_vars[
            'constraint_glitch_peak']
        spice_deck_info[arc_count]['PUSHOUT_PER'] = chartcl_vars[
            'constraint_delay_degrade']
 
        if arc.cell() in chartcl_condtions and 'GLITCH' in chartcl_condtions[
                arc.cell()]:
...
        # add template tcl path
        spice_deck_info[arc_count]['TEMPLATE_TCL'] = template_tcl
        arc_count += 1
 
...
 
 
def arcCharacteristics__getHeaderInfo(arc_info, mpw_input_threshold):
    """
 
    Args:
        arc_info (qaTemplateMaker.classes.ArcInfo):
    """
    arc_info_line = "* " + createQATemplate.getArcInfoLine(
        arc_info)
    slew_info_line = "* " + createQATemplate.getSlewInfoLine(
        arc_info) + ' | MPW_INPUT_RATIO ' + mpw_input_threshold
    template_deck_info_line = "* " + createQATemplate.getTemplateDeckLine(
        arc_info
    )
    header_line = '\n'.join(
...
    log_when = convertLitWhenToLogWhen(lit_when)
...
 
 
def arcCharacteristics__getMaxSlew(template_info):
    """
 
    Args:
        arc_info (qaTemplateMaker.classes.ArcInfo):
    """
    max_slew = template_info._tool_vars['max_transition']
    max_slew = utils.unit_offset_sci_val(max_slew, 9)
    max_slew = '{max_slew:.5f}'.format(max_slew=max_slew)
    max_slew += "n"
    return max_slew
 
 
...
        "falling_edge", "rising_edge", "three_state_disable",
        "three_state_enable", "clear", "preset"
    ]
...
 
 
def convertLitWhenToLogWhen(lit_when):
    if lit_when == "NO_CONDITION":
        log_when = lit_when
    else:
        when_info = lit_when.split('_')
        log_when_list = list()
        for item in when_info:
            if "not" in item:
                log_when_list.append("!%s" % item[3:])
            else:
                log_when_list.append(item)
        log_when = '&'.join(log_when_list)
    return log_when
 
 
...
 
 
def calculate_mpw_input_threshold(chartcl_vars):
    ori_val = chartcl_vars['mpw_input_threshold']
    mpw_input_threshold = 2.5*(float(ori_val) - 0.5)
    try:
        n = len(str(mpw_input_threshold).split('.')[1])
    except:
        n = 4
    mpw_input_threshold = '{thres:.{n}f}'.format(
        n=n, thres=mpw_input_threshold)
    return mpw_input_threshold
 
