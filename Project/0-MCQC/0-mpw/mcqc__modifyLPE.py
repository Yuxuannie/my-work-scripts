...
        print(
            "Output path %s doesn't exist. Creating it now." %
            user_opts['OUTPUT_PATH']
        )
...
        output_lpe_name = "%s_qa.spi" % os.path.basename(input_lpe
                                                        ).split('.spi')[0]
        output_lpe = os.path.join(user_opts['OUTPUT_PATH'], output_lpe_name)
 
        print("Modifying input LPE %s" % input_lpe)
        modifyLPE.createLPEWithParams(
            input_lpe, output_lpe, user_opts['PARAMETER_LIST']
        )
        print("Wrote data to ouput LPE %s" % output_lpe)
    print(
        "Finished modifying LPE. Find output in %s" % user_opts['OUTPUT_PATH']
    )
 
 
def usage():
...
        (
            "--input_path=", ":",
            "The path containing all the input LPE netlists to be "
            "modified."
        ),
        (
            "--output_path=", ":",
            "The path where all the modified LPE netlists will be "
            "stored."
        )
    ]
    print("Arguments:")
...
        (
            "--parameter list=", ":",
            "The list of random variable parameter names, "
...
def parseArgv(argv):
    short_opts = "h"
    long_opts = ["help", "input_path=", "output_path=", "parameter_list="]
 
...
 
    user_opts = dict()
    user_opts['PARAMETER_LIST'] = [
        "parl1", "parl2", "plo_tox", "plo_dxl", "plo_dhfin", "plo_dtfin"
    ]
